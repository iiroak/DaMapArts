...
<div class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
	<div class="flex w-full items-center gap-2">
		<h3 class="flex-1 text-sm font-semibold uppercase tracking-wide text-[var(--color-muted)]">
			<span class="mr-1.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[var(--color-primary)] text-[10px] font-bold leading-none text-white">2</span>
			{t('map.title')}
		</h3>
		<button
	...
	<div class="mt-3 space-y-3">
		...
		{#if staircaseModes.length > 0}
			<label class="block">
				...
				<span class="mt-0.5 block text-[10px] italic text-[var(--color-muted)] opacity-70">{t('map.staircasingHint')}</span>
			</label>
		{/if}
